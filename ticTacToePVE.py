...
def validMove(theBoard, player):
    ...
    return theBoard, playerMove.lower()
...
def pveGame():
    ...
    while turn < 10:
        ...
        availableSpaces = ['a1', 'a2', 'a3', 'b1', 'b2', 'b3', 'c1', 'c2', 'c3']

        
        for i in range(9):
            if player == 'X':
                theBoard, playerMove = validMove(theBoard, player)
                availableSpaces.remove(playerMove)
                print()
            else:
                playerMove = random.choice(availableSpaces)
                theBoard[playerMove.lower()] = player
                availableSpaces.remove(playerMove)
                print()
...
def ticTacToe():
    ...
    while turn < 10:
        ...
        for i in range(9):
            theBoard = validMove(theBoard, player)[0]
            print()
            print('Turn ' + str(turn))
            displayBoard(theBoard)
...
if __name__ == "__main__":
    ...
    while validType != True:
        gameType = input('Would you like a PVP or a PVE Game? ')
        print()
        if gameType.lower() ==  'pvp':
            ticTacToe()
            validType = True
        elif gameType.lower() ==  'pve':
            pveGame()
            validType = True
        else:
            print('Invalid game type selected please try again.')
            print()
